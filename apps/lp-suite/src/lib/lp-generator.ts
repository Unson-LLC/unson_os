...
// デモモード用LP生成関数
function generateDemoLP(prompt: LPGenerationPrompt): Promise<{
  success: boolean;
  ...
  return new Promise((resolve) => {
    // デモ用の遅延（実際のAPI呼び出しをシミュレート）
    setTimeout(() => {
      console.log('🎭 Generating demo LP content...');
      
      const demoConfig: Partial<TemplateConfig> = {
        meta: {
          title: `${prompt.serviceName} | ${prompt.targetAudience}向け最適ソリューション`,
          description: `${prompt.serviceDescription} - ${prompt.mainBenefit}`,
