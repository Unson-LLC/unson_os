    ...
    <section className="section-padding gradient-primary text-white">
      <div className="container-width">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6">{config.title}</h2>
          {config.subtitle && (<p className="text-xl mb-8 text-white/90">{config.subtitle}</p>)}
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="w-full max-w-2xl mx-auto bg-white rounded-xl shadow-xl p-4 sm:p-6 text-left">
              {formConfig && (<FormSection compact config={formConfig} onSubmit={onFormSubmit} prefill={prefill} />)}
            </div>
          </div>
        </div>
      </div>
