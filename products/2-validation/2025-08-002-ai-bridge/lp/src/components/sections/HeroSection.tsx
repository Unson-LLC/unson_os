...
export default function HeroSection({ config, formConfig, onFormSubmit, prefill, onLearnMore }: HeroSectionProps) {
  ...
  React.useEffect(() => {
    ...
    if ((config as any).quickPoints && (config as any).quickPoints.length) {
      trackEvent('quickpoints_view', { count: (config as any).quickPoints.length })
    }
  }, [trackEvent, (config as any).quickPoints])
  return (
    <section 
