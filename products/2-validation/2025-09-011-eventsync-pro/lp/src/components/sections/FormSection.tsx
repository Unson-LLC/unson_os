  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestData = {
        ...
        name: formData.name || '',
        formData,
        source: 'LP-eventsync-pro',
      }
      
      console.log('EventSync Pro - フォーム送信データ:', requestData)
      ...
      console.log('EventSync Pro - APIレスポンス:', result)

      if (!result.success) {
        // 重複登録エラーをユーザーフレンドリーなメッセージに変換
        if (result.error && result.error.includes('既に') && result.error.includes('申し込み済み')) {
          throw new Error('このメールアドレスは既に登録済みです。正式リリース時にご連絡いたします。')
        }
        throw new Error(result.error || 'エラーが発生しました')
      }

      // アナリティクス追跡
      trackFormSubmission('eventsync-pro', 'contact')
