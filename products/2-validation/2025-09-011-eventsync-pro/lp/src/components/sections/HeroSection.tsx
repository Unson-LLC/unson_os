...
export default function HeroSection({ config, assets, onCta }: HeroSectionProps) {
  return (
    <section 
      className="relative min-h-screen flex items-center justify-center overflow-hidden"
      style={{
        backgroundImage: config.backgroundImage ? `url(${config.backgroundImage})` : 
                        (config.image ? `url(${assets?.images ? `${assets.images}/${config.image}` : `/images/${config.image}`})` : undefined),
      ...
      <div className="relative z-10 container-width text-center">
        <div className="max-w-4xl mx-auto animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {config.cta && typeof config.cta === 'object' ? (
              <>
                <button
                  onClick={onCta}
                  className={cn(
