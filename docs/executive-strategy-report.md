...
| 視点 | 従来スタートアップ | **雲孫モデル** |
| --- | --- | --- |
| **プロセス** | 調査 → 企画 → 開発 → QA → リリース → 営業 | Sense → LP → MVP → Billing を **24 h – 8 h** サイクルで自動旋回 |
...
1. **パイプライン安定稼働**
    - “課題 → 課金” リードタイム **≤ 8 時間**
2. **プロダクトポートフォリオ規模**
    - **月次 20 件**の新規 LP 投入、課金まで到達するプロダクトを **累計 50 本**
3. **財務指標**
...
- 年間ローンチ数 1,000+、ポートフォリオ価値 **ARR 100 億円**規模へ。
- “営業レスで事業が量産される” 新しい産業カテゴリ（**Self-Running Business Factory**）の覇者を目指す。
...
| **即座に出現** | LP / MVP / Billing を 8 h サイクルで回す | サイクル数／週 |
| **人⇄AI 協調** | 人間は Go / Hold と倫理ガードのみ | 人的介入時間／サイクル |

---

### 2-3　Core Values ─ ４つの価値観
...
| **Community Flywheel** | *「稼いだら還す、還したら増える」* | 売上の 30% を DAO 配当にプール |
...
| **速度** | 課題→課金リードタイム 8 h | P95 サイクル時間 ≤ 8 h |
| **ポートフォリオ** | 新規 LP 20 件／月 | フェーズ3到達プロダクト 4 本／月 |
...
| **CI/CD** | GitHub Actions → Cloudflare Workers／Neon(DB) に自動デプロイ |
| **メディア要素** | 新しいUI画像・動画が必要な場合は AI 生成 → **デザイナー確認** |
| **ゲート②指標** | ① 週次利用者200人以上 ② 7日後残存率30%以上 |
| **想定リードタイム** | コードは24時間以内、メディア確認が入る場合でも48時間以内 |
...
| クリック率（CTR） | 表示に対するクリック割合 | 1.5％以上 | 30 分 |
...
| **運用基盤** | GitHub Actions（CI/CD）Vercel・Cloudflare Workers（実行環境）Neon／D1（DB） | 自動テスト・本番反映無停止スケールデータ保存 | テスト失敗時の再実行判断 |
...
| **実行環境** | - **LP**：Vercel（静的＋ISR）- **MVP**：Cloudflare Workers（無停止） |
...
| 項目 | 比率 | 説明 |
| --- | --- | --- |
| 運営・投資 | 70 % | インフラ・人件費・再投資 |
| コミュニティ配当 | 30 % | コントリビュータへ四半期配当 |
...
| --- | --- |
| 発行総量 | 1 億枚（追加発行なし） |
| 初期配分 | 70 % 雲孫GK（2年線形ロック）20 % Community配当原資10 % 予備 |
...
| 項目 | 措置 |
| --- | --- |
| 法人格 | 親：雲孫合同会社（日本）DAO：Wyoming DAO LLC |
...
| 30–60日 | Stripe Webhook を 60/10/30 振分に更新／Snapshot 試験投票 |
| 60–90日 | 初回 Founder Bonus 支払／Fund に30 % プール開始 |
...
- **60/10/30** 分流で「固定費」「創業報酬」「全体配当」をコードで完全分離。
