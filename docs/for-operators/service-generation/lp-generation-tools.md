...
### 検証LPの設計ポリシー（合意事項）
- 検証フェーズのLPでは「料金・プラン表示」は出さない
  - 目的はベータ参加/関心登録の獲得を最優先にするため
  - 価格訴求はMVP以降（Stripe連携以降）に移行
- フォームは最小限（メールアドレス中心）。FAQ・社会的証明は有効
- 分析はPostHog＋GAを併用。ヒートマップとスクロール深度を重視
- 4時間周期の自動最適化（キーワード等）は人の事前承認なしでOK（後日レビュー）

参考: プレイブック PB-001（LP CVRテスト）に計測・判定の詳細があります
